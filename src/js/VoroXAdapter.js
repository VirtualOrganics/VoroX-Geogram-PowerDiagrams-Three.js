...
export async function createVoroX({ Module, points, periodic=true, centering='circumcenter' }) {
  ...
  function step(dt, options = {}, scores) {
    const {
      useEdgeMode = false,
      // XPBD path disabled in Power-Diagram Part 1 scaffold
      useXPBD = false,
      edgeScores = null,
      edgeScale = false,
    ...
    let g = null;
    // Physical XPBD path disabled; fall back to gradient-only
    if (false && useXPBD && useEdgeMode && edgeScores && edgeScores.size > 0) {
